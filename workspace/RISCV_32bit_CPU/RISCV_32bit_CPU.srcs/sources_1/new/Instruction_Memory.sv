module Instruction_Memory (
    ...
    initial begin
        ...
        rom[4] = 32'b0100000_00001_00010_101_00111_0110011; // SRA  x7, x2, x1   ; x7 = 2 >>> 1 = 1 (산술 시프트)
        ...
        // L-Type (rs1 = x2 = 2, offset = 4)
        rom[10] = 32'b000000000100_00010_000_01000_0000011; // LB   x8, 4(x2)    ; x8 = M[6][0:7]
        rom[11] = 32'b000000000100_00010_001_01000_0000011; // LH   x8, 4(x2)    ; x8 = M[6][0:15]
        rom[12] = 32'b000000000100_00010_010_01000_0000011; // LW   x8, 4(x2)    ; x8 = M[6][0:31]
        rom[13] = 32'b000000000100_00010_100_01000_0000011; // LBU  x8, 4(x2)    ; x8 = M[6][0:7], zero-extend
        rom[14] = 32'b000000000100_00010_101_01000_0000011; // LHU  x8, 4(x2)    ; x8 = M[6][0:15], zero-extend
        ...
        rom[22] = 32'b0000000_00100_00010_101_01010_0010011; // SRLI  x10, x2, 4  ; x10 = 2 >> 4 = 0
        rom[23] = 32'b0100000_00100_00010_101_01010_0010011; // SRAI  x10, x2, 4  ; x10 = 2 >>> 4 = 0

        // S-Type (rs1 = x2 = 2, rs2 = x1 = 1, imm = 4)
        rom[24] = 32'b0000000_00001_00010_000_00100_0100011; // SB  x1 → 4(x2)    ; M[6] = x1[7:0]
        rom[25] = 32'b0000000_00001_00010_001_00100_0100011; // SH  x1 → 4(x2)    ; M[6] = x1[15:0]
        rom[26] = 32'b0000000_00001_00010_010_00100_0100011; // SW  x1 → 4(x2)    ; M[6] = x1

        // B-Type (offset = 8 → PC + 8 → +2 instruction)
        rom[29] = 32'b0000000_00011_00010_000_01000_1100011; // BEQ  x2, x3, 8    ; 2 == 2 → 참 → jump to 31
        rom[30] = 32'b0000000_00011_00100_001_01000_1100011; // BNE  x4, x3, 8    ; 3 != 2 → 참 → jump to 32
        rom[31] = 32'b0000000_00101_00110_100_01000_1100011; // BLT  x5, x6, 8    ; -3 < -4 → 거짓 → 다음으로
        rom[32] = 32'b0000000_00110_00101_101_01000_1100011; // BGE  x6, x5, 8    ; -4 ≥ -3 → 거짓 → 다음으로
        rom[33] = 32'b0000000_00100_00011_110_01000_1100011; // BLTU x3, x4, 8    ; 2 < 3 → 참 → jump to 35
        rom[34] = 32'b0000000_00011_00100_111_01000_1100011; // BGEU x4, x3, 8    ; 3 ≥ 2 → 참 → jump to 36
        ...
        // J-Type
        rom[35] = 32'b00000000100000000000_01011_1101111;   // JAL x11, 8      ; x11 = PC+4, PC += 8
        rom[36] = 32'b000000000000_00000_000_01011_1100111; // JALR x11, 0(x0)   ; x11 = PC+4, PC = x0 + 0 → 0
        rom[37] = 32'b000001111000_00000_000_01011_1100111; // JALR x11, 120(x0)   ; x11 = PC+4, PC = x0 + 120 → 120

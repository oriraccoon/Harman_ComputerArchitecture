...
class transaction;
    ...
    task display(string name);
        $display(
            "[%S] oper=%h, wdata=%h, wr_en=%d, full=%h, rdata=%h, rd_en=%d, empty=%h",
...
class driver;
    ...
    task write();
        ...
        fifo_if.drv_cb.wr_en <= fifo_tr.wr_en;
        fifo_if.drv_cb.rd_en <= 1'b0;
        //@(posedge fifo_if.cb.clk);
        @(fifo_if.drv_cb);
        ...
        forever begin
            ...
            if (fifo_tr.rd_en == 1'b1) begin
                if (fifo_tr.empty == 1'b0) begin
                    ...
                    $display("[SCB] : PASS!! Matched Data! ref_model: %h == rData: %h",
                             pop_data, fifo_tr.rdata);
                        
                    end
                    else begin
                        $display(
                            "[SCB] : FAIL!! Dismatched Data! ref_model: %h != rData: %h",
                            pop_data, fifo_tr.rdata);
                    end
                end
                else begin

module Gaussian_Blur (
    ...
    assign o_data = {guass_sum[3:0], guass_sum[3:0], guass_sum[3:0]};
    ...
    always_ff @( posedge clk or posedge reset ) begin : window_out
        if (reset) begin
            ...
            if (pipe_valid[2]) begin
                guass_sum <=    ((gauss_window[0][2] << GAUSS_WINDOW[0][2]) + 
...
module Laplasian_Filter (
    ...
    assign l_data = {cliping_sum, cliping_sum, cliping_sum};
        ...
        if (reset) begin
            ...
            if (pipe_valid[2]) begin
                Laplacian_sum <= ((Laplacian_window[0][2]) + 
                                  ...
                                  (Laplacian_window[1][1] *  LAPLACIAN_WINDOW[1][1]) +
